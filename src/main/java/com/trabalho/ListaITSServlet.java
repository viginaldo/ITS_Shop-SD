...
public class ListaITSServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        estoque.put("8", new Equipamento("8", "Webcam 1080p Logitech C920 (1080p)", 2500.00));
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        for (Equipamento e : estoque.values()) {
            ...
            out.println(
                    " <button class='btn btn-sm position-absolute top-0 end-0 m-2 bg-white rounded-circle' onclick=\"toggleCart('"
                            + e.getId() + "')\">");
            out.println(" <i class='fas fa-cart-plus text-success' id='icon-" + e.getId() + "'></i>");
            out.println(" </button>");
            out.println(" <img src='" + cp + "/imagens/" + e.getId() + ".jpg' class='card-img-top' alt='"
                    + e.getNome() + "'>");
            out.println(" </div>");
            ...
            out.println(
                    " <button class='btn btn-success btn-sm mt-2' data-bs-toggle='offcanvas' data-bs-target='#offcanvasCompra' onclick=\"abrirOffcanvas('"
                            + e.getId() + "')\">Comprar</button>");
            out.println(" </div>");
            out.println(" </div>");
            out.println(" </div>");
        }
        ...
        out.println(" <div class='col text-center'><img src='" + cp
                + "/imagens/devs/langa.jpg' width='70' class='rounded-circle'><br>Patrick Langa<br>20231003</div>");
        out.println(" <div class='col text-center'><img src='" + cp
                + "/imagens/devs/ryry.jpg' width='70' class='rounded-circle'><br>Ryazy Hassane<br>20230304</div>");
        out.println(" <div class='col text-center'><img src='" + cp
                + "/imagens/devs/tig.jpg' width='70' class='rounded-circle'><br>Tiago Correia<br>20200018</div>");
        out.println(" <div class='col text-center'><img src='" + cp
                + "/imagens/devs/vigi.jpg' width='70' class='rounded-circle'><br>Viginaldo Joaquim<br>20210982</div>");
        out.println(" <div class='col text-center'><img src='" + cp
                + "/imagens/devs/yuyu.jpg' width='70' class='rounded-circle'><br>Yunus Suelmia<br>20210382</div>");
        out.println(" </div>");
        out.println(" </div>");
        out.println(" </footer>");
        ...
        out.println(
                " <button class='btn btn-primary rounded-circle position-fixed' style='bottom:90px;right:20px;width:60px;height:60px;z-index:1000;' data-bs-toggle='modal' data-bs-target='#modalCarrinho'>");
        ...
        out.println(
                " <button onclick='topFunction()' id='backToTop' class='btn btn-dark rounded-circle back-to-top'>");
        ...
        out.println(
                " <button class='btn btn-primary w-100 mb-3' onclick='mostrarFormOffcanvas()'>Continuar</button>");
        ...
        out.println(
                " <div class='mb-3'><label class='form-label'>Nome Completo</label><input type='text' class='form-control' name='nomeCliente' required></div>");
        out.println(
                " <div class='mb-3'><label class='form-label'>Endereço Completo</label><input type='text' class='form-control' name='endereco' required></div>");
        out.println(" <div class='row mb-3'>");
        out.println(" <div class='col-md-8'>");
        out.println(" <label class='form-label'>Número do Cartão</label>");
        out.println(
                " <input type='text' class='form-control numero-cartao' name='numeroCartao' maxlength='19' placeholder='1234 5678 9012 3456' required>");
        out.println(" </div>");
        out.println(" <div class='col-md-4'>");
        out.println(" <label class='form-label'>CVV</label>");
        out.println(
                " <input type='text' class='form-control' name='cvv' pattern='\\d{3,4}' maxlength='4' placeholder='123'>");
        out.println(" </div>");
        out.println(" </div>");
        ...
        out.println(
                " <div class='form-check'><input class='form-check-input' type='checkbox' id='selecionar-todos' onchange='toggleAll()'> <label class='form-check-label'>Todos</label></div>");
        out.println(
                " <button class='btn btn-sm btn-outline-danger' onclick='removerSelecionados()'>Remover</button>");
        out.println(" </div>");
        ...
        out.println(" <div class='modal-content'>");
        out.println(
                " <div class='modal-header'><h5>Finalizar Compra</h5><button type='button' class='btn-close' data-bs-dismiss='modal'></button></div>");
        ...
        out.println(
                " const ic = document.getElementById('icon-'+id); if(ic) ic.className = 'fas fa-cart-plus text-success';");
        out.println(" } else {");
        out.println(" cart[id] = 1;");
        out.println(
                " const ic = document.getElementById('icon-'+id); if(ic) ic.className = 'fas fa-check text-primary';");
        out.println(" }");
        ...
        out.println(" }");
        out.println("");
        // CORREÇÃO AQUI: REMOVIDO preventDefault() + addEventListener direto no form
        out.println(" document.getElementById('formOffcanvas').addEventListener('submit', function(e) {");
        ...
        out.println("");
        out.println(
                " // construir elemento via DOM (evita problemas com template literals dentro de out.println)");
        ...
        out.println(
                " info.innerHTML = '<strong>' + item.nome + '</strong><br><small>' + item.preco + ' MT</small>'; ");
        out.println("");
        ...
        out.println(
                " nomeDiv.innerHTML = \"<label class='form-label'>Nome Completo</label><input type='text' class='form-control' name='nomeCliente' required>\";");
        out.println(" form.appendChild(nomeDiv);");
        out.println("");
        out.println(" const endDiv = document.createElement('div');");
        out.println(" endDiv.className = 'mb-3';");
        out.println(
                " endDiv.innerHTML = \"<label class='form-label'>Endereço Completo</label><input type='text' class='form-control' name='endereco' required>\";");
        ...
        out.println(
                " cartInput.type = 'text'; cartInput.name = 'numeroCartao'; cartInput.className = 'form-control numero-cartao';");
        out.println(
                " cartInput.maxLength = 19; cartInput.placeholder = '1234 5678 9012 3456'; cartInput.required = true;");
        ...
        out.println(
                " cvvInput.pattern = '\\d{3,4}'; cvvInput.maxLength = 4; cvvInput.placeholder = '123';");
        ...
        out.println("");
        out.println(
                " const totalHidden = document.createElement('input'); totalHidden.type = 'hidden'; totalHidden.name = 'total'; totalHidden.value = total.toFixed(2); form.appendChild(totalHidden);");
        out.println("");
        out.println(
                " const btn = document.createElement('button'); btn.type = 'submit'; btn.className = 'btn btn-success w-100'; btn.textContent = 'Confirmar'; form.appendChild(btn);");
        out.println("");
        out.println(" // fecha modal carrinho e abre modal checkout");
        out.println(
                " const modalCarrinhoInst = bootstrap.Modal.getInstance(document.getElementById('modalCarrinho'));");
